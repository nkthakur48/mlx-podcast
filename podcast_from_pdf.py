...
# Maps emotion tag → TTS speed multiplier.
# Kokoro is also sensitive to punctuation: "..." pauses, "!" energises, CAPS stresses.
...
HOST_A: [surprised] Wait, really? I assumed the whole point was throughput.
...
def synthesize_lines(lines, clip_dir: Path):
    ...
    for i, (voice, emotion, text) in enumerate(lines):
        speed = EMOTION_SPEED.get(emotion, DEFAULT_SPEED)
        ...
        print(f"  {i+1}/{len(lines)}  [{name:4s}][{emotion:10s}]  {text[:55]}")
    return files
